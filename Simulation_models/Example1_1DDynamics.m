...
classdef Example1_1DDynamics < Dynamics
    ...
    % Constructor
    function obj = Example1_1DDynamics(alpha, sigma_squared)
        obj.alpha = alpha;
    ...
    function A_t = A_t(obj,x ,~ ,dt)
        ...
        A_t = eye(length(x)) - obj.alpha .* sin(x) .* dt;
    end
    ...
    function math_F_out = math_F(obj , x, ~, V_x)
        math_F_out = -obj.alpha .* cos(x) * V_x;
